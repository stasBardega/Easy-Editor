...
btn_increase_brightness = QPushButton("Збільшити яскравість")
btn_saturation = QPushButton("Насиченість")
...
h2.addWidget(btn_increase_brightness)
h2.addWidget(btn_saturation)
...
class ImageProcessor():
    ...
    def increase_brightness(self):
        ...
        current_brightness *= 1.1  # Збільшити яскравість на 10%
        enhancer = ImageEnhance.Brightness(self.image)
        self.image = enhancer.enhance(current_brightness)
        self.saveImage()
        self.showImage()

    def do_saturation(self, factor=1.5):
...
list_file.setStyleSheet('''color: white;background-color:blue;''')
...
btn_increase_brightness.setStyleSheet('''color: black;background-color:yellow;''')
btn_file.setStyleSheet('''color: black;background-color:lightgreen;''')
btn_saturation.setStyleSheet('''color: black;background-color: yellow;''')
...
def save_image_as():
    if workimage.image:  # Якщо є завантажене зображення
        save_path, _ = QFileDialog.getSaveFileName(win, "Зберегти зображення", "", "Images (*.png *.jpg *.bmp)")
        if save_path:
            workimage.image.save(save_path)  # Зберігаємо зображення за вказаним шляхом
...
btn_increase_brightness.clicked.connect(workimage.increase_brightness)

btn_saturation.clicked.connect(workimage.do_saturation)


exit_shortcut = QShortcut(QKeySequence("Ctrl+Q"), win)
exit_shortcut.activated.connect(exit_app)

save_shortcut = QShortcut(QKeySequence("Ctrl+S"), win)
save_shortcut.activated.connect(save_image_as)
